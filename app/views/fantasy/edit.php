<div class="page-header">
    <h1>✏️ Editar equipo — <?= htmlspecialchars($league['name']) ?></h1>
...
<div class="budget-bar card">
    <div class="budget-info">
        <span>💰 Presupuesto: <strong id="budget-remaining">0</strong>M€ restantes</span>
        <span>Pilotos: <strong id="drivers-count">0</strong>/<?= MAX_DRIVERS ?></span>
        <span>Equipos: <strong id="teams-count">0</strong>/<?= MAX_TEAMS ?></span>
    </div>
</div>

<form method="POST" action="<?= BASE_URL ?>/fantasy/update" id="fantasy-form">
    ...
    <?php foreach ($drivers as $d): ?>
        <?php $checked = in_array($d['id'], $selDrivers); ?>
        <label class="pick-card<?= $checked ? ' selected' : '' ?>" data-price="<?= $d['price'] ?>" data-type="driver">
            <input type="checkbox" name="drivers[]" value="<?= htmlspecialchars($d['id']) ?>"
                   class="pick-cb driver-cb"<?= $checked ? ' checked' : '' ?>>
    ...
    <?php foreach ($teams as $t): ?>
        <?php $checked = in_array($t['id'], $selTeams); ?>
        <label class="pick-card<?= $checked ? ' selected' : '' ?>" data-price="<?= $t['price'] ?>" data-type="constructor">
            <input type="checkbox" name="constructors[]" value="<?= htmlspecialchars($t['id']) ?>"
                   class="pick-cb team-cb"<?= $checked ? ' checked' : '' ?>>
    ...
    <div class="form-submit mt-lg">
        <button type="submit" class="btn btn-primary btn-lg" id="submit-btn">Guardar cambios</button>
        <a href="<?= BASE_URL ?>/fantasy?league=<?= $league['id'] ?>" class="btn btn-secondary">Cancelar</a>
    </div>
</form>
...
(function() {
    ...
    // Initialise from pre-checked selections
    document.querySelectorAll('.pick-cb:checked').forEach(cb => {
        const card = cb.closest('.pick-card');
        const price = parseFloat(card.dataset.price);
        spent += price;
        if (card.dataset.type === 'driver') driversCount++; else teamsCount++;
    });
    ...
    document.querySelectorAll('.pick-cb').forEach(cb => {
        cb.addEventListener('change', function() {
            ...
            const price = parseFloat(card.dataset.price);
            const isDriver = card.dataset.type === 'driver';
